    ...
    <>
      ...
      <div className="min-h-screen bg-background">
        <div className="max-w-3xl mx-auto px-6 py-8">
          <Link href="/paths" className="inline-flex items-center gap-1 text-white/40 hover:text-white text-sm"><ArrowLeft className="h-3.5 w-3.5" /> All paths</Link>
          ...
          <div className="mt-4 bg-surface-elevated border border-border rounded-2xl p-5">
            ...
            <div className="mt-4 flex items-center gap-3 flex-wrap">
              <button onClick={adapt} disabled={busy === 'adapt'} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-medium disabled:opacity-50">
                <span className="inline-flex items-center gap-1.5"><Sparkles className="h-4 w-4" /> {busy === 'adapt' ? 'Adapting…' : 'Adapt to my performance'}</span>
              </button>
            ...
            {p.modules.map((m) => (
              <div key={m.id} className={`flex items-center justify-between gap-3 rounded-xl px-4 py-3 border ${m.status === 'completed' ? 'border-green-500/30 bg-green-500/5' : 'border-border bg-surface-elevated'}`}>
                <div className="min-w-0">
                  <p className="flex items-center gap-1.5 text-white text-sm truncate">
                    {m.status === 'completed' && <Check className="h-4 w-4 flex-shrink-0 text-green-400" />}{m.module_number}. {m.title}
                  </p>
