...
// Primary links sit directly in the bar; the rest live under a "More" dropdown
// so the navbar doesn't overflow as features grow.
const PRIMARY_LINKS = [
...
export default function Navbar({ user, isAdmin, onLogout }: NavbarProps) {
  const moreLinks = isAdmin ? [...MORE_LINKS, ADMIN_LINK] : MORE_LINKS;
  const navLinks = [...PRIMARY_LINKS, ...moreLinks]; // mobile shows everything
          ...
          {user && (
            <div className="hidden md:flex items-center gap-1">
              ...
              <div className="relative" ref={moreRef}>
                ...
                {moreOpen && (
                  <div className="absolute right-0 mt-1 w-48 bg-[#1c1c1c] border border-white/10 rounded-xl overflow-hidden shadow-2xl z-50 py-1">
                    {moreLinks.map((link) => (
                      <Link
                        key={link.href}
                        href={link.href}
                        className={`block px-4 py-2 text-sm transition-colors ${
                          isActive(link.href) ? "text-accent" : "text-white/70 hover:text-white hover:bg-white/5"
                        }`}
                      >
                        {link.label}
                      </Link>
